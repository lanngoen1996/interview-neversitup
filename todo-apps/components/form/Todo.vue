...
const create = async () => {
  try {   
    await useAPI<{ data: TodoResponse }>('/todo', {
      method: 'POST',
      body: form.value
    })
    layoutStore.toggleForm()
  } catch (error) {
    console.error(error)
  }
}
      ...
      <div class="px-5">
        ...
        <div class="mt-5">
          ...
          <textarea 
            ...
            name="description" 
            :placeholder="message.form.description.placeholder" 
            rows="5" 
            class="w-full border-2 rounded mt-3 p-2 focus:outline-none" 
          />
        </div>
        <div class="mt-5">
          <p class="base text-gray-500 font-semibold">
